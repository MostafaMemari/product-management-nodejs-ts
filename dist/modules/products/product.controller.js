...
class ProductController {
    ...
    async create(req, res, next) {
        try {
            ...
            const productDto = (0, class_transformer_1.plainToClass)(product_dto_1.ProductDTO, req.body, { excludeExtraneousValues: true, exposeUnsetFields: false });
    ...
    async update(req, res, next) {
        try {
            ...
            const productDto = (0, class_transformer_1.plainToClass)(product_dto_1.ProductUpdateDTO, req.body, { excludeExtraneousValues: true, exposeUnsetFields: false });
            productDto.color ? await this.colorService.checkExistColor({ id: productDto.color }) : false;
            productDto.category ? await this.categoryService.checkExistCategory({ id: productDto.category }) : false;
            productDto.seller ? await this.sellerService.checkExistSeller({ id: productDto.seller }) : false;
    ...
    async updateRobot(req, res, next) {
        try {
            ...
            const productDto = (0, class_transformer_1.plainToClass)(product_dto_1.ProductRobotDTO, req.body, { excludeExtraneousValues: true, exposeUnsetFields: false });
    ...
    async findAllProductAndSumSellBuy(req, res, next) {
        try {
            const query = (0, class_transformer_1.plainToClass)(product_dto_1.ProductQueryDTO, req.query, { excludeExtraneousValues: true, exposeUnsetFields: false });
            const colors = await this.colorService.find();
            const categories = await this.categoryService.find();
...
class ProductControllerEJS {
    ...
    async create(req, res, next) {
        try {
            ...
            productDto.seller ? await this.sellerService.checkExistSeller({ id: productDto.seller }) : false;
            await this.service.create(productDto, req.file);
            req.flash("success", product_message_1.ProductMessage.Created);
            res.redirect("/panel/products");
        }
        catch (error) {
            req.flash("error", product_message_1.ProductMessage.ErrorCreate);
            res.redirect("/panel/products");
            // next(error);
        }
    }
    async update(req, res, next) {
        try {
            ...
            productDto.seller ? await this.sellerService.checkExistSeller({ id: productDto.seller }) : false;
            await this.service.update(productID, productDto, req.file);
            req.flash("success", product_message_1.ProductMessage.Updated);
            res.redirect("/panel/products");
        }
        catch (error) {
            req.flash("error", product_message_1.ProductMessage.ErrorUpdate);
            res.redirect("/panel/products");
            // next(error);
        }
    }
