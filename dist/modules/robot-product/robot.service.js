...
const robot_model_1 = require("./robot.model");
...
class ProductService {
    async create(productDto, reqFile) {
        ...
        const product = await robot_model_1.ProductModel.create({ ...productDto, img });
        return product;
    }
    async update(productID, productDto, reqFile) {
        ...
        const result = await robot_model_1.ProductModel.updateOne({ _id: productID.id }, { ...productDto, img });
    ...
    async updateRobot(productID, productDto) {
        (0, error_handler_1.errorHandler)({ productID, productDto });
        const result = await robot_model_1.ProductModel.updateOne({ _id: productID.id }, { robot: { ...productDto } });
        if (!result.modifiedCount)
            throw http_errors_1.default.InternalServerError();
        return true;
    }
    ...
    async find(query, colorsDto, categoryDto, sellerDto) {
        ...
        const products = await robot_model_1.ProductModel.find({ title: { $regex: search, $options: "i" } })
            .where("category")
            .in(categories)
            .where("color")
            .in(colors)
            ...
            .lean();
        const total = await robot_model_1.ProductModel.countDocuments({
    ...
    async defects(query) {
        ...
        const search = query.search || "";
        const sort = query.sort == "asc" ? "asc" : "desc" || "desc";
        const products = await robot_model_1.ProductModel.find({
            title: { $regex: search, $options: "i" },
    ...
    async removeByID(productID) {
        (0, error_handler_1.errorHandler)({ productID });
        const product = await this.checkExistProduct(productID);
        const deletedProduct = await robot_model_1.ProductModel.deleteOne({ _id: product === null || product === void 0 ? void 0 : product._id });
    ...
    async checkExistProduct(productID) {
        const product = await robot_model_1.ProductModel.findById(productID.id).lean();
        if (!product)
            throw http_errors_1.default.NotFound(robot_message_1.ProductMessage.NotFound);
        return product;
    }
    async checkExistProductDKPC(dkpc) {
        const product = await robot_model_1.ProductModel.findOne({ dkpc: dkpc.dkpc });
        if (product)
            throw http_errors_1.default.Conflict(robot_message_1.ProductMessage.Conflict);
        return product;
    }
}
