...
class ColorService {
    async create(colorDto) {
        ...
        const color = await buy_model_1.ColorModel.create(colorDto);
        return color;
    }
    async update(colorID, colorDto) {
        ...
        const result = await buy_model_1.ColorModel.updateOne({ _id: colorID.id }, { ...colorDto });
    ...
    async find() {
        const color = await buy_model_1.ColorModel.find({});
        return color;
    }
    async removeByID(colorID) {
        (0, error_handler_1.errorHandler)({ colorID });
        const color = await this.checkExistColor(colorID);
        const deletedcolor = await buy_model_1.ColorModel.deleteOne({ _id: color === null || color === void 0 ? void 0 : color.id });
        if (!deletedcolor.deletedCount)
            throw http_errors_1.default.InternalServerError();
        return true;
    }
    async checkExistColor(colorID) {
        const color = await buy_model_1.ColorModel.findById(colorID.id);
        if (!color)
